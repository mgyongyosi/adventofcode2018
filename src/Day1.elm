...
calculateFirstPuzzle start lst = 
    List.foldl (+) start lst
        
calculateSecondPuzzle : List Int -> Int -> List Int -> Int
calculateSecondPuzzle lst startSum freqList =
    let
        nextSum = calculateFirstPuzzle startSum lst
        previousFrequencyValues = (freqList ++ (listOfFrequencies lst startSum)) |> Set.fromList
        currentIterationFrequencyValues = listOfFrequencies lst nextSum
        value = currentIterationFrequencyValues |> List.Extra.find (\val -> Set.member val previousFrequencyValues)
    in
        case value of
            Just num -> num
            Nothing -> calculateSecondPuzzle lst nextSum ((Set.toList previousFrequencyValues) ++ currentIterationFrequencyValues)
...
main = 
     div []
        ...
        h1 [] [ text ("Second: " ++ (calculateSecondPuzzle input 0 [] |> String.fromInt)) ]
        ]
